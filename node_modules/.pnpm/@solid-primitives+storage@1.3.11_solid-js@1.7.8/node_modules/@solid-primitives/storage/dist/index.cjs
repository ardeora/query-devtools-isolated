...
function createStorage(props) {
  ...
  const store = new Proxy(
    {},
    {
      get(_, key) {
        ...
        if (!node) {
          node = solidJs.createSignal(void 0, { equals: false });
  ...
  props?.sync !== false && solidJs.onMount(() => {
    const listener = (ev) => {
      let changed = false;
      apis.forEach((api) => {
        try {
    ...
    if ("addEventListener" in globalThis) {
      ...
      solidJs.onCleanup(() => globalThis.removeEventListener("storage", listener));
    } else {
      apis.forEach((api) => api.addEventListener?.("storage", listener));
      solidJs.onCleanup(() => apis.forEach((api) => api.removeEventListener?.("storage", listener)));
    }
  });
  return [
    store,
...
function createStorageSignal(key, initialValue, props) {
  ...
  const [accessor, setter] = utils.createHydrateSignal(
    initialValue,
    () => read() ?? initialValue,
    props
  );
...
exports.createLocalStorage = createStorage;
exports.createSessionStorage = (props) => createStorage({ ...props, api: globalThis.sessionStorage });

// src/tools.ts
exports.addClearMethod = (storage) => {
...
exports.cookieStorage = exports.addClearMethod({
  _cookies: [globalThis.document, "cookie"],
  getItem: (key) => exports.cookieStorage._cookies[0][exports.cookieStorage._cookies[1]].match("(^|;)\\s*" + key + "\\s*=\\s*([^;]+)")?.pop() ?? null,
  setItem: (key, value, options) => {
    const oldValue = exports.cookieStorage.getItem(key);
    exports.cookieStorage._cookies[0][exports.cookieStorage._cookies[1]] = `${key}=${value}${serializeCookieOptions(
      options
    )}`;
    const storageEvent = Object.assign(new Event("storage"), {
      ...
      storageArea: exports.cookieStorage
    });
    window.dispatchEvent(storageEvent);
  },
  removeItem: (key) => {
    exports.cookieStorage._cookies[0][exports.cookieStorage._cookies[1]] = `${key}=deleted${serializeCookieOptions({
      expires:  new Date(0)
    })}`;
  },
  key: (index) => {
    let key = null;
    let count = 0;
    exports.cookieStorage._cookies[0][exports.cookieStorage._cookies[1]].replace(
      /(?:^|;)\s*(.+?)\s*=\s*[^;]+/g,
      (_, found) => {
...
exports.createCookieStorage = (props) => createStorage({ ...props, api: exports.cookieStorage });

exports.createAsyncStorage = createAsyncStorage;
exports.createStorage = createStorage;
exports.createStorageSignal = createStorageSignal;
